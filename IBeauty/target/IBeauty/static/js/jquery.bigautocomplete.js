...
(function($){
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			$(".search").append("<div id='bigAutocompleteContent' class='searchBox'></div>");
			...
			$("#bigAutocompleteContent").delegate("li", "mouseover", function() {
				$("#bigAutocompleteContent li").removeClass("selected");
			...
			$("#bigAutocompleteContent").delegate("li", "click", function() {
				bigAutocomplete.currentInputText.val( $(this).find("a:last").html());
				var callback_ = bigAutocomplete.currentInputText.data("config").callback;
				if($("#bigAutocompleteContent").css("display") != "none" && callback_ && $.isFunction(callback_)){
					var link = $(this).attr("data-link");
					var d = $(this).data("jsonData");
					d["link"] = link;
					callback_(d);
		...
		this.autocomplete = function(param){
			...
			if($(".search").length > 0 && $("#bigAutocompleteContent").length <= 0){
...
//					$bigAutocompleteContent.css({"top":offset.top + h,"left":offset.left});
			...
			function makeContAndShow(data_){
				...
				for(var i=0;i<data_.length;i++){
					cont += "<li><a href='javascript:;'>" + data_[i].title + "</a></li>"
				}
				cont += "</ul>";
				...
				$bigAutocompleteContent.find("li").each(function(index){
					...
					//只能用于搜索
					$(this).attr("data-link",data_[index].link);
				})
			}			
